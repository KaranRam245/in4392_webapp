...
class Instances:
    ...
    def set_state(self, instance_id, instance_type, state):
        ...
        if instance_id not in nodes:
            self.logger.log_info("instances", "State of instance " + instance_id + " set to PENDING.")
            nodes[instance_id] = InstanceState(InstanceState.PENDING)
        self.logger.log_info("instances", "State of instance " + instance_id + " set to " + str(state) + ".")
        nodes[instance_id] = state

    def set_ip(self, instance_id, ip_address):
        self.logger.log_info("instances", "IP address of " + instance_id + " set to " + ip_address + ".")
...
class NodeScheduler:
    ...
    def initialize_nodes(self, retry=False):
        ...
        if self.debug and not self.node_manager_running:
            ...
            return False
        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Initializing nodes..")
        print("Initializing nodes..")
        if self.instances.has_instance_not_running(instance_type='node_manager'):
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "No node manager running. Intializing startup protocol..")
        ...
        if self.instances.has_instance_not_running(instance_type='worker'):
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "No single worker running. Intializing startup protocol..")
    ...
    def _send_start_command(self, instance_type, instance_id):
        try:
            ...
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "Sending start command: [{}]: {}.".format(instance_id, command))
            ...
            self.logger.log_exception("nodescheduler-" + str(self.instance_id), "The following exception has occurred while trying"
    ...
    def start_node_manager(self):
        if not self.instances.has('node_manager', [InstanceState.RUNNING, InstanceState.PENDING]):
            ...
            if not nodemanagers:
                self.logger.log_error("nodescheduler-" + str(self.instance_id), "No node manager instances available to start.")
                raise ConnectionError('No node manager instances available to start.')
            to_start = nodemanagers[0]
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "Initializing node manager.")
    ...
    def start_worker(self):
        ...
        if not workers:
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "No more worker instances can be started.")
            print('No more worker instances can be started.')
            return
        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Initializing worker.")
    ...
    def _init_instance(self, instance_id: int, instance_type: str, wait=False):
        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Starting {} instance {}".format(instance_type, instance_id))
    ...
    def update_instances(self, check=True):
        ...
        if check and not (
            ...
            return
        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Updated instance states from AWS state.")
    ...
    async def run(self):
        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Running NodeScheduler..")
        ...
        try:
            ...
            while self.debug and not initialized:
                # self.logger.log_warning("nodescheduler-" + str(self.instance_id), "Debug enabled and no node manager started yet. "
    ...
    def _check_living(self, instance, instance_type):
        ...
        if self.instances.start_signal_timedout(instance):
            ...
            self.logger.log_error("nodescheduler-" + str(self.instance_id), "No/timedout heartbeat recorded "
        ...
        if send_start:  # Send a new start signal to the instance.
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "Sent start command to instance {}".format(instance))
    ...
    def cancel_all(self):
        ...
        if running_instances:
            self.logger.log_info("nodescheduler-" + str(self.instance_id), "Killing all instances: {}".format(running_instances))
            print("Killing all instances: {}".format(running_instances))
            self.boto.ec2.stop_instances(InstanceIds=running_instances)

        self.logger.log_info("nodescheduler-" + str(self.instance_id), "Cancelling all commands..")
...
def start_instance(debug=False, git_pull=False):
    ...
    """
    logger = Logger()
    logger.log_info("instancemanager", "Starting Node Scheduler..")

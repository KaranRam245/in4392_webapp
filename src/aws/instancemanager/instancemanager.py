...
class NodeScheduler:
    ...
    def initialize_nodes(self, retry=False):
        ...
        if self.debug and not self.node_manager_running:
            self.logger.log_info("nodescheduler-" + self.instance_id, "Debugging waiting for node manager to start running.")
            print("Debugging waiting for node manager to start running.")
            return False
        self.logger.log_info("nodescheduler-" + self.instance_id, "Initializing nodes..")
        print("Initializing nodes..")
        if self.instances.has_instance_not_running(instance_type='node_manager'):
            self.logger.log_info("nodescheduler-" + self.instance_id, "No node manager running. Intializing startup protocol..")
        ...
        if self.instances.has_instance_not_running(instance_type='worker'):
            self.logger.log_info("nodescheduler-" + self.instance_id, "No single worker running. Intializing startup protocol..")
    ...
    def start_worker(self):
        ...
        if not workers:
            self.logger.log_info("nodescheduler-" + self.instance_id, "No more worker instances can be started.")
            print('No more worker instances can be started.')
            return
        self.logger.log_info("nodescheduler-" + self.instance_id, "Initializing worker.")
    ...
    def _init_instance(self, instance_id: int, instance_type: str, wait=False):
        self.logger.log_info("nodescheduler-" + instance_id, "Starting {} instance {}".format(instance_type, instance_id))
    ...
    def update_instances(self, check=True):
        ...
        if check and not (
            ...
            return
        self.logger.log_info("nodescheduler-" + self.instance_id, "Updated instance states from AWS state.")
    ...
    async def run(self):
        self.logger.log_info("nodescheduler-" + self.instance_id, "Running NodeScheduler..")
    ...
    def _check_living(self, instance, instance_type):
        ...
        if send_start:  # Send a new start signal to the instance.
            self.logger.log_info("nodescheduler-" + self.instance_id, "Sent start command to instance {}".format(instance))
    ...
    def cancel_all(self):
        ...
        if running_instances:
            self.logger.log_info("nodescheduler-" + self.instance_id, "Killing all instances: {}".format(running_instances))

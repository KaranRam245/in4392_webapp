...
selectedModel=st.sidebar.selectbox("Please select the model you would like to use",available_models)
...
if trainModel and selectedModel is 'Conv 1D':
    st.write(f"Model training with following parameters - Max sequence length: {MAX_SEQUENCE_LENGTH},Max Vocab Size: {MAX_VOCAB_SIZE}")

    trainingPath=os.path.join(os.curdir,'src','data','train','train.csv')
    data,targets,word2idx= Tokenize.train_tokenizer(trainingPath,MAX_VOCAB_SIZE,MAX_SEQUENCE_LENGTH)
    embedding_matrix= GloveObject.fillPretrainedEmbeddings(word2idx,MAX_VOCAB_SIZE)
    num_words = min(MAX_VOCAB_SIZE, len(word2idx) + 1)
    callbacks=[ModelCheckpoint(os.path.join(models_storage,'Conv1D.h5'),monitor='val_acc',save_best_only=True)]
    model=ConvNet1D()
    model.set_pretrained_embeddings(num_words,EMBEDDING_DIM,embedding_matrix)

    model.compile(optimizer='adam',loss='binary_crossentropy',metrics=['acc'])
    history=model.fit(data,targets,batch_size=BATCH_SIZE,epochs=EPOCHS,validation_split=VALIDATION_SPLIT,callbacks=callbacks)
    ...
    embedding_matrix= GloveObject.fillPretrainedEmbeddings(word2idx,MAX_VOCAB_SIZE)
    num_words = min(MAX_VOCAB_SIZE, len(word2idx) + 1)
    callbacks=[ModelCheckpoint(os.path.join(models_storage,'LSTM.h5'),monitor='val_acc',save_best_only=True)]
    model=LSTMBuild()
    model.set_pretrained_embeddings(num_words,EMBEDDING_DIM,embedding_matrix)
